...
import type {DiffOperation, Memory} from "./models.js";
import {z} from "zod";
...
const OpSchema = z.array(z.object({
...
export interface Consolidator {
    ...
    consolidate(memories: Memory[], evidenceIds: string[], now: string): Promise<DiffOperation[]>;
}
...
export class RuleConsolidator implements Consolidator {
    ...
    async consolidate(memories: Memory[], evidenceIds: string[], now: string): Promise<DiffOperation[]> {
        const live = memories.filter((m) => !m.tombstone);
        const ev = evidenceIds.length ? evidenceIds : live.map((m) => m.sourceId ?? m.id).slice(0, 1);
        ...
        for (const m of live) {
            ...
            if (hit) hit.ids.push(m.id); else seen.push({key: words, ids: [m.id], text: m.text});
        }
        for (const g of seen) {
            if (g.ids.length >= 2) {
                ops.push({type: "consolidate", mergeIds: g.ids, into: {text: g.text}, confidence: 0.9, evidence: ev});
            }
        }
        return ops;
    }
...
export class AnthropicConsolidator implements Consolidator {
    ...
    async consolidate(memories: Memory[], evidenceIds: string[], _now: string): Promise<DiffOperation[]> {
        const body = `# Current memory\n${memories.filter((m) => !m.tombstone).map((m) => `[${m.id}] ${m.text}`).join("\n")}\n\n# Evidence ids\n${evidenceIds.join(", ")}`;
        ...
        if (!res.ok) throw new Error(`anthropic consolidate: ${res.status} ${await res.text()}`);
...
export function createConsolidator(cfg: Config): Consolidator {
    return cfg.models.anthropicApiKey ? new AnthropicConsolidator(cfg) : new RuleConsolidator();
}
