...
import {applyDiff, getExtraction, type IngestInput, ingestSource, runDream, verify} from "./sync.js";
...
export class Cortex {
    ...
    ingestText(text: string, title?: string) {
        return this.ingest({type: "note", uri: "inline://" + (title ?? "note"), title, text, hint: title});
    }
    ...
    async digest(period: { from: string; to: string } = {from: "0000", to: "9999"}) {
