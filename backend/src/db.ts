...
export function createClients(cfg: Config): Clients {
    return {
        walrus: createWalrus(cfg), seal: createSeal(cfg), memwal: createMemWal(cfg), sui: createSui(cfg), cfg,
    };
}

/** encode -> Seal-encrypt -> Walrus write. */
export async function putArtifact(c: Clients, artifact: Artifact): Promise<string> {
...
export async function getArtifact<T extends Artifact = Artifact>(c: Clients, blobId: string, kind?: T["kind"]): Promise<T> {
...
export async function fetchRaw(c: Clients, blobId: string): Promise<Uint8Array> {
