...
import { stateHash } from "./crypto.js";
...
class MockSui implements SuiClient {
  ...
  async recordManifest(namespace: string, manifestBlobId: string) {
    this.ptr.set(namespace, { manifestBlobId, suiObject: "0xobj_" + stateHash([namespace]) });
...
class LiveSui implements SuiClient {
  ...
  async recordManifest(namespace: string, manifestBlobId: string): Promise<string> {
    ...
    const res = await c.signAndExecuteTransaction({ signer: await this.signer(), transaction: tx });
    return res.digest;
  }
  async readManifestPointer(namespace: string): Promise<ManifestPointer> {
    ...
    throw new Error(`sui(live): registry not deployed for ${namespace}; set seal.policyPackage`);
  }
}

export function createSui(cfg: Config): SuiClient {
  return isLive(cfg) && cfg.seal.policyPackage ? new LiveSui(cfg) : new MockSui();
}
