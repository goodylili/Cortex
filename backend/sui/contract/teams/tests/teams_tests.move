...
use cortex::{access::{Self, AccessRegistry, ExecutorCap}, account::{Self, Account, Registry}};
...
fun setup(scenario: &mut ts::Scenario) {
    ...
    ts::next_tx(scenario, OWNER);
    {
        ...
        let clk = clock::create_for_testing(ts::ctx(scenario));
        team::create_team(&acct, string::utf8(b"Acme Corp"), &clk, ts::ctx(scenario));
        clock::destroy_for_testing(clk);
...
fun add(scenario: &mut ts::Scenario, member: address, role: u8) {
    ts::next_tx(scenario, OWNER);
    {
        ...
        let clk = clock::create_for_testing(ts::ctx(scenario));
        team::add_member_by_address(&mut team, member, string::utf8(b"bob.cortex.sui"), role, &clk, ts::ctx(scenario));
        clock::destroy_for_testing(clk);
...
fun add_member_by_address_records_role() {
    ...
    ts::next_tx(&mut scenario, OWNER);
    {
        ...
        assert!(team.member_role(MEMBER).destroy_some() == team::role_member(), 3);
        assert!(team.member_name(MEMBER).destroy_some() == string::utf8(b"bob.cortex.sui"), 4);
        ts::return_shared(team);
    };
...
fun add_member_by_handle_resolves_address() {
    ...
    ts::next_tx(&mut scenario, OWNER);
    {
        ...
        team::add_member_by_handle(&mut team, &registry, string::utf8(b"bob"), team::role_admin(), &clk, ts::ctx(&mut scenario));
        assert!(team.is_member(MEMBER), 0);
        assert!(team.is_admin(MEMBER), 1);
        assert!(team.member_name(MEMBER).destroy_some() == string::utf8(b"bob.cortex.sui"), 2);
        clock::destroy_for_testing(clk);
        ts::return_shared(team);
        ts::return_shared(registry);
    };
...
fun set_role_promotes_member() {
    ...
    ts::next_tx(&mut scenario, OWNER);
    {
        let mut team = ts::take_shared<Team>(&scenario);
        let clk = clock::create_for_testing(ts::ctx(&mut scenario));
        team::set_role(&mut team, MEMBER, team::role_admin(), &clk, ts::ctx(&mut scenario));
...
fun non_admin_cannot_add_member() {
    ...
    ts::next_tx(&mut scenario, STRANGER);
    {
        let mut team = ts::take_shared<Team>(&scenario);
        let clk = clock::create_for_testing(ts::ctx(&mut scenario));
        team::add_member_by_address(&mut team, STRANGER, string::utf8(b"evil"), team::role_admin(), &clk, ts::ctx(&mut scenario));
        clock::destroy_for_testing(clk);
        ts::return_shared(team);
    };
...
fun owner_cannot_re_add_self() {
    ...
    ts::next_tx(&mut scenario, OWNER);
    {
        let mut team = ts::take_shared<Team>(&scenario);
        let clk = clock::create_for_testing(ts::ctx(&mut scenario));
        team::add_member_by_address(&mut team, OWNER, string::utf8(b"acme"), team::role_member(), &clk, ts::ctx(&mut scenario));
        clock::destroy_for_testing(clk);
        ts::return_shared(team);
    };
...
fun member_can_seal_approve_active_team() {
    ...
    {
        let team = ts::take_shared<Team>(&scenario);
        team::seal_approve_for_testing(scoped_identity(&team), &team, ts::ctx(&mut scenario));
        ts::return_shared(team);
    };
...
fun member_can_set_feed_blob() {
    ...
    ts::next_tx(&mut scenario, MEMBER);
    {
        let mut team = ts::take_shared<Team>(&scenario);
        let clk = clock::create_for_testing(ts::ctx(&mut scenario));
        team::set_feed_blob(&mut team, string::utf8(b"blob-abc"), &clk, ts::ctx(&mut scenario));
...
fun executor_can_set_memory_blob() {
    ...
    ts::next_tx(&mut scenario, OWNER);
    {
        ...
        let mut team = ts::take_shared<Team>(&scenario);
        let clk = clock::create_for_testing(ts::ctx(&mut scenario));
        team::executor_set_memory(&registry, &cap, &mut team, string::utf8(b"mem-blob"), &clk);
