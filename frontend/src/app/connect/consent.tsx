// Managed-OAuth consent UI. Client-only (loaded with ssr:false) because it pulls
// the wallet -> Walrus wasm + Privy SDK, which can't be evaluated during SSR /
// prerender. The MCP server's /authorize redirects here with the OAuth request;
// the user signs in (Privy wallet) and approves once. On approval we run the
// on-chain grant + sign the challenge (wallet.connectMcp), hand the proof to the
// MCP's /oauth/grant, and follow the returned redirect back to the client. The
// wallet stays in the browser; the MCP only ever gets a signature.
