...
export type Facet = "health" | "identity" | "work" | "finance" | "relationships" | "casual" | "ephemeral_task";
...
export const FACET_MIN: Record<Facet, Tier> = {
  health: 3, identity: 3, work: 2, finance: 2, relationships: 2, casual: 0, ephemeral_task: 0,
};

export const TIER_NAME: Record<Tier, string> = {
  0: "Ephemeral", 1: "Temporary", 2: "Standard", 3: "Durable", 4: "Core",
};
export const FACETS: Facet[] = ["health", "identity", "work", "finance", "relationships", "casual", "ephemeral_task"];

// JSON turns Infinity into null. Bring it back so the engine stays exact.
export function normalizeConfig(c: Partial<MemoryConfig> | null | undefined): MemoryConfig {
  const base = { ...DEFAULT_CONFIG, ...(c || {}) };
  const fix = (r: Record<number, number>, d: Record<number, number>) => {
    const out = { ...d } as Record<Tier, number>;
    ([0, 1, 2, 3, 4] as Tier[]).forEach((t) => { const v = r?.[t]; out[t] = v == null ? d[t]! : v; });
    return out;
  };
  return {
    ...base,
    halfLife: fix(base.halfLife as any, DEFAULT_CONFIG.halfLife),
    floor: fix(base.floor as any, DEFAULT_CONFIG.floor),
    ttl: fix(base.ttl as any, DEFAULT_CONFIG.ttl),
  };
}
// For persistence: Infinity -> null so it round-trips.
export function serializeConfig(c: MemoryConfig): any {
  const strip = (r: Record<Tier, number>) => {
    const o: Record<number, number | null> = {};
    ([0, 1, 2, 3, 4] as Tier[]).forEach((t) => { o[t] = r[t] === INF ? null : r[t]; });
    return o;
  };
...
const FACET_FROM_TAG: Record<string, Facet> = {
  health: "health", money: "finance", people: "relationships", work: "work",
};
const IDENTITY_RE = /\b(my name is|i am|i'm|i live|i was born|allergic|allergy|i prefer|i always|i never)\b/i;
const HEALTH_RE = /\b(allergic|allergy|asthma|diabet|blood type|medication|doctor|diagnos)\b/i;
const RULE_RE = /\b(always|never|from now on|i prefer|please remember|make sure|must|don'?t ever)\b/i;
const CORRECTION_RE = /\b(no,? actually|not |i told you|that'?s wrong|correction|it'?s actually|i meant)\b/i;
const TRANSIENT_RE = /\b(today|right now|currently|for now|this morning|tired|just |temporarily)\b/i;
...
export function facetOf(text: string, tags: string[]): Facet {
  ...
  for (const t of tags) { const f = FACET_FROM_TAG[t]; if (f) return f; }
...
export function scoreTier(text: string, facet: Facet, importance: "low" | "normal" | "high" | undefined): Tier {
  let tier: number = importance === "high" ? 3 : importance === "low" ? 1 : 2;
  if (RULE_RE.test(text) || HEALTH_RE.test(text) || IDENTITY_RE.test(text)) tier = Math.max(tier, 3);
...
export function isCorrection(text: string): boolean { return CORRECTION_RE.test(text); }

// Backfill model fields onto a legacy memory. Idempotent.
export function ensureModel(m: Memory, now: number): Memory {
  ...
  let lock: Lock = (m.lock as Lock) ?? (m.kept ? "pinned" : m.tombstone ? "suppressed" : "none");
  if (m.kept && tier < 3) tier = 3 as Tier;
  return {
    ...m,
    tier, facet, lock,
    trust: (m.trust as Trust) ?? (m.source === "note" || m.source === "reflection" ? "stated" : "inferred"),
...
export function newMemory(base: Memory, importance: "low" | "normal" | "high" | undefined, trust: Trust): Memory {
  ...
  return {
    ...
    tier, facet, trust,
    activation: 0.35,
    lastAccess: now, accessCount: 0,
    lock: tier >= 3 && importance === "high" ? "pinned" : "none",
    decayPenalty: trust === "inferred" ? 1.0 : 1.0,
    edges: [], rawRef: `walrus://local/${base.id}`, accessLog: [],
  };
}

// ---- the retention engine ----

export function decayedActivation(m: Memory, now: number, c: MemoryConfig): number {
  ...
  const penalty = (m.decayPenalty ?? 1) * (m.trust === "inferred" ? c.inferredPenalty : 1);
...
export function isRetrievable(m: Memory, now: number, c: MemoryConfig): boolean {
  if (m.lock === "pinned") return true;
  if (m.lock === "suppressed" || m.lock === "forgotten_hard") return false;
  return retention(m, now, c) >= c.theta;
}

export function isForgettable(m: Memory, now: number, c: MemoryConfig): boolean {
...
export type MemState = "pinned" | "core" | "active" | "fading" | "forgotten" | "purged";
...
export function onAccess(m: Memory, now: number, c: MemoryConfig): Memory {
  const log = [...(m.accessLog ?? []), now].filter((t) => now - t < 30 * DAY).slice(-20);
  return { ...m, activation: Math.min(1, (m.activation ?? 0.3) + c.accessBump), lastAccess: now, accessCount: (m.accessCount ?? 0) + 1, accessLog: log };
}
export function like(m: Memory, now: number): Memory {
  const tier = Math.min(4, (m.tier ?? 2) + 1) as Tier;
  return { ...m, tier, lock: "pinned", lockedAt: now, trust: "stated", kept: true };
}
export function dislike(m: Memory, now: number): Memory {
  const tier = Math.max(0, (m.tier ?? 2) - 1) as Tier;
  return { ...m, tier, activation: 0, decayPenalty: 2.0, lock: "none", lockedAt: now, kept: false };
}
export function rememberPin(m: Memory, now: number, core = false): Memory {
  return { ...m, tier: (core ? 4 : 3) as Tier, lock: "pinned", lockedAt: now, trust: "stated", kept: true };
}
export function forget(m: Memory, now: number, hard = false): Memory {
  if (hard) return { ...m, lock: "forgotten_hard", lockedAt: now, content: "", text: "", rawRef: null, tombstone: true };
...
export function restore(m: Memory, now: number): Memory {
  ...
  return { ...m, lock: "none", lockedAt: now, activation: Math.max(m.activation ?? 0, 0.35), lastAccess: now, tombstone: false };
}

// ---- the daily sweep (consolidation) ----

export interface SweepSummary { deindexed: number; promoted: number; demoted: number; merged: number; linked: number; scanned: number }
...
export function consolidate(input: Memory[], now: number, c: MemoryConfig): { memories: Memory[]; summary: SweepSummary } {
  let memories = input.map((m) => ensureModel(m, now));
  const summary: SweepSummary = { deindexed: 0, promoted: 0, demoted: 0, merged: 0, linked: 0, scanned: memories.length };

  // 1+2. de-index everything forgettable (retention below theta and past TTL)
  memories = memories.map((m) => {
    if (isForgettable(m, now, c)) { summary.deindexed++; return { ...m, lock: "suppressed" as Lock, note: "faded below recall" }; }
    return m;
  });

  // 3. dedup near-identical active memories (fold the shorter into the longer)
  const active = memories.filter((m) => (m.lock ?? "none") === "none" && !m.tombstone);
  ...
  for (let i = 0; i < active.length; i++) {
    for (let j = i + 1; j < active.length; j++) {
      ...
      if (similar(a, b) >= 0.6) {
        const keep = a.text.length >= b.text.length ? a : b, drop = keep === a ? b : a;
        folded.add(drop.id);
  ...
  if (folded.size) memories = memories.map((m) => (folded.has(m.id) ? { ...m, lock: "suppressed" as Lock, note: "folded into a near-identical memory" } : m));
  ...
  memories = memories.map((m) => {
    ...
    const recent = (m.accessLog ?? []).filter((t) => now - t < c.promoteWindowDays * DAY).length;
    const facetMin = FACET_MIN[(m.facet as Facet) ?? "casual"];
    if (recent >= c.promoteAccesses && (m.tier ?? 2) < 4) { summary.promoted++; return { ...m, tier: Math.min(4, (m.tier ?? 2) + 1) as Tier }; }
    if (recent === 0 && retention(m, now, c) < c.theta + 0.1 && (m.tier ?? 2) > facetMin) { summary.demoted++; return { ...m, tier: Math.max(facetMin, (m.tier ?? 2) - 1) as Tier }; }
    return m;
  });

  // 6. rebuild associative edges between related active memories
  const live = memories.filter((m) => (m.lock ?? "none") !== "suppressed" && m.lock !== "forgotten_hard" && !m.tombstone);
  const edgeMap: Record<string, string[]> = {};
  for (let i = 0; i < live.length; i++) {
    for (let j = i + 1; j < live.length; j++) {
      const a = live[i]!, b = live[j]!;
      const sharedTag = a.tags.some((t) => t !== "note" && b.tags.includes(t));
      if (sharedTag || similar(a, b) >= 0.3) { (edgeMap[a.id] ||= []).push(b.id); (edgeMap[b.id] ||= []).push(a.id); summary.linked++; }
    }
  }
  memories = memories.map((m) => (edgeMap[m.id] ? { ...m, edges: [...new Set([...(m.edges ?? []), ...edgeMap[m.id]!])].slice(0, 8) } : m));
