...
export const FORMATS: { id: PromptFormat; name: string; sub: string; file: string }[] = [
...
export interface CompileInput { task: string; memories: Memory[] }
...
export function compilePrompt(format: PromptFormat, { task, memories }: CompileInput): string {
  const goal = task.trim() || "Help me using what I already know.";
  const prov = provenance(memories);
  switch (format) {
    case "system":
      return [
        "# System prompt", "", "## What I need", goal, "",
        "## What I already know (treat as ground truth, cite when used)",
        ...memories.map((m) => `- ${m.text}`), "",
        `<!-- ${prov} -->`,
      ].join("\n");
    case "markdown":
      return [`# Briefing`, ``, `**Task:** ${goal}`, ``, `## What I know`, ...memories.map((m) => `- ${m.text}`), ``, `> ${prov}`].join("\n");
    case "xml":
      return [
        `<context source="cortex">`, `  <task>${goal}</task>`, `  <memories>`,
        ...memories.map((m) => `    <memory${m.kept ? ' kept="true"' : ""}>${m.text}</memory>`),
        `  </memories>`, `  <!-- ${prov} -->`, `</context>`,
      ].join("\n");
    case "json":
      return JSON.stringify({ kind: "cortex.prompt", task: goal, memories: memories.map((m) => ({ text: m.text, tags: m.tags, kept: !!m.kept })), provenance: prov }, null, 2);
    case "claudemd":
      return [`# CLAUDE.md`, ``, `> Synced from Cortex. ${prov}`, ``, `## What this is about`, goal, ``, `## Things I know`, ...memories.map((m) => `- ${m.text}`)].join("\n");
  }
}
